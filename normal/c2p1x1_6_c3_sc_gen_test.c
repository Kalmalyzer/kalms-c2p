...
UTEST(normal, c2p1x1_6_c3_sc_gen) {
	...
	for (int y = 0; y < chunkyy; y++)
		for (int x = 0; x < chunkyx; x++)
			tempbuf[256 * 256 * 2 + y * chunkyx + ((x & 7) << 2) + ((x >> 3) & 3) + (x & ~0x1f)] = random_320x256x6bpl_chunky[y * chunkyx + x];

	memset(tempbuf, 0, bplsize * depth);

	c2p1x1_6_c3_sc_gen_init(chunkyx, chunkyy, scroffsy);
	c2p1x1_6_c3_sc_gen(tempbuf + 256 * 256 * 2, tempbuf);
